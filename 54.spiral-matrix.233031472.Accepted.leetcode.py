class Solution(object):
    ...
    def get_spiralOrder(self, matrix, r_start, r_end, c_start, c_end):
        ...
        curr = matrix[r_start][c_start:c_end + 1] + [matrix[j][c_end] for j in range(r_start + 1, r_end)] + matrix[r_end][c_start:c_end + 1][::-1] + [matrix[j][c_start] for j in reversed(range(r_start + 1, r_end))]
